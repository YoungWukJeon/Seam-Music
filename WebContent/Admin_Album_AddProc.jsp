...
<%
	...
	String album_no = ad.getEnabledAlbumNo();
	
	ServletContext context = getServletContext();
	String saveFolder="image/album_image/";
	String realFolder = context.getRealPath(saveFolder);
	String encType = "utf-8";
	int sizeLimit = 1000 * 1024 * 1024;
	String default_img = "default_albumart.png";
	String input_img = saveFolder + album_no;
	...
	String release_com = multi.getParameter("release_com");
	...
	if( files.hasMoreElements() )
	{
		...
		if( filename.equals("albumart") )
		{
			if( str != null )
			{
				String newName = album_no + str.substring(str.lastIndexOf("."));
			...
			else
			{
				try
				{
					...
					FileOutputStream fos = new FileOutputStream(realFolder + "/" + album_no + ".png");
	...
	ad.addAlbum(album_no, nation_no, name, artist_no, release_date, release_com, agency, input_img);

	// response.sendRedirect("Admin_Album.jsp?filter=" + filter + "&filter_content=" + filter_content);
	
	response.sendRedirect("Admin_Album_Add.jsp?filter=" + filter + "&filter_content=" + filter_content);
